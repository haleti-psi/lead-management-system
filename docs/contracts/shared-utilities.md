...
|---|---|---|
| `LeadService` | M2 | **sole writer of `leads`** (no other module/service may UPDATE/INSERT `leads`): `create / transitionStage / assignOwner / bulkReassign / setScore / setHotFlag / setKycStatus / setConsentStatus / setSlaDueAt / recordEligibility / markHandedOff / merge`. Single-row mutators take `expectedVersion` (stale → `CONFLICT`) **except the volatile derived-field mutators `setScore` (FR-011), `setConsentStatus` (FR-110) and `recomputeDuplicateStatus` (FR-020)** — these take NO `expectedVersion` and do NOT bump `version` (system-managed volatile fields must not raise false 409s against concurrent human edits; they write audit but no optimistic lock); **`assignOwner` uses the FR-030 options-object form** (returns `AssignOwnerResult`; stage-preserving on reassign past `assigned` — see `modules/capture/lead.service.ts`; write-back per FR-030 LLD); **`merge`/`unmerge` use the FR-021 options-object form** (dual `expectedVersion`s, precedence + manual overrides, E3 `relinked_ids` in audit detail; write-back per FR-021); `bulkReassign(leadIds, ownerId, reason, tx)` is the LIMIT-bounded admin path (FR-130) — bumps `version` per row, writes one `audit_logs(reassign)` per lead, no per-row `expectedVersion`; **`markHandedOff(leadId, losApplicationId, expectedVersion, actorId, tx)`** — FR-081 write-back: sets stage to `handed_off`, writes stage_history, audit (HANDOFF_SUCCESS), and LEAD_HANDED_OFF outbox event |
